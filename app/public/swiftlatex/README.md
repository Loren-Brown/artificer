...
- `PdfTeXEngine.js` — API wrapper (`ENGINE_PATH` is relative `swiftlatex/swiftlatexpdftex.js`; `setTexliveEndpoint` no longer drops the worker)
...
These ship under `public/swiftlatex/` so Vite serves them at `{base}swiftlatex/*` (with `base` `/artificer/`, that is `/artificer/swiftlatex/*`).
